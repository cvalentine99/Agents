...
async function startServer() {
      ...
      try {
        ...
        switch (data.type) {
          case "create": {
            ...
            ptyService.createSession(sessionId!, data.userId || "anonymous", cwd);
            ...
            outputHandler = (sid: string, output: string) => {
              if (sid === currentSessionId && ws.readyState === 1) {
                ws.send(JSON.stringify({ type: "output", data: output, timestamp: Date.now() }));
              }
            };
            
            exitHandler = (sid: string, exitCode: number) => {
              if (sid === currentSessionId && ws.readyState === 1) {
                ws.send(JSON.stringify({ type: "exit", exitCode, timestamp: Date.now() }));
              }
            };
          ...
          case "kill":
            ...
            if (sessionId) {
              ...
              ws.send(JSON.stringify({ type: "killed", sessionId, timestamp: Date.now() }));
            }
            break;
        }
      } catch (error) {
        ws.send(JSON.stringify({ type: "error", error: String(error), timestamp: Date.now() }));
      }
    });
        ...
        switch (data.type) {
          case "start":
            ...
            ralphEngine.on("stateChange", (sid: string, state: unknown) => {
              if (sid === sessionId && ws.readyState === 1) {
                ws.send(JSON.stringify({ type: "state", data: state, timestamp: Date.now() }));
              }
            });
            
            ralphEngine.on("iterationComplete", (sid: string, iteration: unknown) => {
              if (sid === sessionId && ws.readyState === 1) {
                ws.send(JSON.stringify({ type: "iteration", data: iteration, timestamp: Date.now() }));
              }
            });
            
            ralphEngine.on("complete", (sid: string, state: unknown) => {
              if (sid === sessionId && ws.readyState === 1) {
                ws.send(JSON.stringify({ type: "complete", data: state, timestamp: Date.now() }));
              }
            });
  ...
  server.listen(port, () => {
    ...
    console.log(`WebSocket CLI stream available at ws://localhost:${port}/api/ws/cli`);
    console.log(`WebSocket PTY terminal available at ws://localhost:${port}/api/ws/pty`);
    console.log(`WebSocket RALPH engine available at ws://localhost:${port}/api/ws/ralph`);
  });
}
