...
export async function addCompletionCriterion(
  criterion: InsertCompletionCriterion
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getSessionCriteria(sessionId: number) {
  ...
  return db
    ...
    .where(eq(completionCriteria.sessionId, sessionId));
}
...
export async function getSessionMetrics(sessionId: number) {
  ...
  return db
    .select()
    .from(loopMetrics)
    .where(eq(loopMetrics.sessionId, sessionId))
    .orderBy(loopMetrics.iteration);
}
...
export async function getSessionAssemblyRuns(sessionId: number) {
  ...
  return db
    .select()
    .from(assemblyLineRuns)
    .where(eq(assemblyLineRuns.sessionId, sessionId))
    .orderBy(desc(assemblyLineRuns.createdAt));
}

export async function updateAssemblyLineRun(
  id: number,
  updates: Partial<InsertAssemblyLineRun>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  return db
    .update(assemblyLineRuns)
    .set(updates)
    .where(eq(assemblyLineRuns.id, id));
}
...
export async function getSessionDiffHunks(sessionId: number) {
  ...
  return db
    .select()
    .from(diffHunks)
    .where(eq(diffHunks.sessionId, sessionId))
    .orderBy(desc(diffHunks.createdAt));
}
...
// API Keys and CLI Executions imports consolidated at top
  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(apiKeys.provider, apiKey.provider)
      )
    )
    .limit(1);
...
export async function updateCliExecution(
  id: number,
  updates: Partial<InsertCliExecution>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getSessionCliExecutions(sessionId: number) {
  ...
  return db
    .select()
    .from(cliExecutions)
    .where(eq(cliExecutions.sessionId, sessionId))
    .orderBy(desc(cliExecutions.startedAt));
}
...
export async function getRunningExecutions() {
  ...
  return db
    .select()
    .from(cliExecutions)
    .where(eq(cliExecutions.status, "running"));
}

// ==================== SESSION TEMPLATES QUERIES ====================

// Session Templates imports consolidated at top
...
export async function getSessionTemplateById(id: number, userId: number) {
  ...
  const result = await db
    ...
    .from(sessionTemplates)
    .where(
      and(eq(sessionTemplates.id, id), eq(sessionTemplates.userId, userId))
    )
    .limit(1);
...
export async function updateSessionTemplate(
  id: number,
  userId: number,
  updates: Partial<InsertSessionTemplate>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
// Research imports consolidated at top
...
export async function updateResearchSession(
  id: number,
  updates: Partial<InsertResearchSession>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function deleteResearchSession(id: number) {
  ...
  await db
    ...
    .where(eq(researchFindings.researchSessionId, id));
...
export async function getResearchFindings(researchSessionId: number) {
  ...
  return db
    ...
    .where(eq(researchFindings.researchSessionId, researchSessionId))
    .orderBy(researchFindings.stepNumber);
}
...
export async function getResearchSteps(researchSessionId: number) {
  ...
  return db
    .select()
    .from(researchSteps)
    .where(eq(researchSteps.researchSessionId, researchSessionId))
    .orderBy(researchSteps.stepNumber);
}

export async function updateResearchStep(
  id: number,
  updates: Partial<InsertResearchStep>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getResearchByShareToken(shareToken: string) {
  ...
  const result = await db
    .select()
    .from(researchSessions)
    .where(eq(researchSessions.shareToken, shareToken))
    .limit(1);
  return result.length > 0 ? result[0] : undefined;
}
...
export async function getResearchFollowUps(researchSessionId: number) {
  ...
  return db
    ...
    .where(eq(researchFollowUps.researchSessionId, researchSessionId))
    .orderBy(researchFollowUps.createdAt);
}

export async function updateResearchFollowUp(
  id: number,
  updates: Partial<InsertResearchFollowUp>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  return db
    .update(researchFollowUps)
    .set(updates)
    .where(eq(researchFollowUps.id, id));
}
...
export async function updateCustomTemplate(
  id: number,
  userId: number,
  updates: Partial<InsertCustomTemplate>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getUserCategory(id: number, userId: number) {
  ...
  const result = await db
    ...
    .where(
      and(
        eq(userTemplateCategories.id, id),
        eq(userTemplateCategories.userId, userId)
      )
    )
    .limit(1);
  return result.length > 0 ? result[0] : undefined;
...
export async function updateUserCategory(
  id: number,
  userId: number,
  updates: Partial<InsertUserTemplateCategory>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  return db
    .update(userTemplateCategories)
    .set({ ...updates, updatedAt: new Date() })
    .where(
      and(
        eq(userTemplateCategories.id, id),
...
export async function deleteUserCategory(id: number, userId: number) {
  ...
  await db
    ...
    .where(
      and(
        eq(customTemplates.categoryId, id),
...
export async function exportUserTemplates(
  userId: number
    ...
    try {
      ...
      const existing = await db
        ...
        .where(
          and(
            eq(customTemplates.userId, userId),
            eq(customTemplates.name, template.name)
          )
        )
        .limit(1);
...
export async function getCustomAgentProfile(id: number, userId: number) {
  ...
  const result = await db
    ...
    .where(
      and(
        eq(customAgentProfiles.id, id),
        eq(customAgentProfiles.userId, userId)
      )
    )
    .limit(1);
  return result.length > 0 ? result[0] : undefined;
...
export async function updateCustomAgentProfile(
  id: number,
  userId: number,
  updates: Partial<InsertCustomAgentProfile>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  return db
    .update(customAgentProfiles)
    .set(updates)
    .where(
      and(
        eq(customAgentProfiles.id, id),
