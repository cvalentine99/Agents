...
describe("Crypto Module", () => {
  describe("encrypt/decrypt", () => {
    ...
    it("should use utf8 encoding for input and output", () => {
      ...
      expect(Buffer.from(decrypted, "utf8").length).toBe(
        Buffer.from(originalText, "utf8").length
      );
    });
    ...
    it("should fail decryption with tampered ciphertext", () => {
      ...
      const tamperedEncrypted =
        encrypted.slice(0, 96) + "ff" + encrypted.slice(98);

      try {
        const result = decrypt(tamperedEncrypted);
    ...
    it("should fail decryption with tampered auth tag", () => {
      ...
      const tamperedAuthTag =
        encrypted.slice(0, 64) + "00".repeat(16) + encrypted.slice(96);
