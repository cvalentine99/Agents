...
class RalphEngine extends EventEmitter {
  private sessions: Map<string, { config: RalphConfig; state: RalphState; running: boolean }> = new Map();
  ...
  async startSession(config: RalphConfig): Promise<void> {
    ...
    this.emit('log', config.sessionId, `🚀 RALPH Loop started for ${config.workingDir}`);
    this.emit('log', config.sessionId, `📋 Goal: ${config.prompt.goal}`);
    this.emit('log', config.sessionId, `🤖 Model: ${config.model}`);
    this.emit('log', config.sessionId, `🔄 Max iterations: ${config.maxIterations}`);
  ...
  private async runLoop(sessionId: string): Promise<void> {
    ...
    while (session.running && state.currentIteration < config.maxIterations) {
      ...
      try {
        ...
        const codeResult = await this.generateChanges(sessionId, config, currentState);

        if (codeResult.success && codeResult.code) {
          // Step 3: Apply changes
          this.emit('log', sessionId, '📝 Applying changes...');
          const applyResult = await this.applyChanges(sessionId, config, codeResult);
          
          if (applyResult.filesChanged.length > 0) {
            state.filesModified = Array.from(new Set([...state.filesModified, ...applyResult.filesChanged]));
          }
          ...
          const completionCheck = await this.checkCompletion(sessionId, config, testResult);
          ...
          if (completionCheck.madeProgress) {
            ...
            this.emit('log', sessionId, `⚠️ No progress detected (${state.noProgressCount}/${config.noProgressThreshold})`);
            
            if (state.noProgressCount >= config.noProgressThreshold) {
              state.circuitBreaker = 'OPEN';
              this.emit('log', sessionId, '🔴 Circuit breaker OPENED - too many iterations without progress');
            }
          }
        } else {
          ...
          if (!isNaN(sessionIdNum)) {
            autoSign.recordFailure(sessionIdNum, errorMsg, state.currentIteration);
            this.emit('log', sessionId, '📊 Failure recorded for auto-sign analysis');
          }
        }
        ...
        if (!isNaN(sessionIdNum)) {
          ...
          const consecutiveFailures = autoSign.getConsecutiveFailures(sessionIdNum);
          if (consecutiveFailures >= 2) {
            const suggestions = autoSign.getAutoSuggestions(sessionIdNum);
            if (suggestions.length > 0) {
              this.emit('log', sessionId, `💡 ${suggestions.length} sign suggestion(s) available - check AI Signs tab`);
            }
          }
        }
      }
   ...
   */
  private async analyzeProjectState(sessionId: string, config: RalphConfig): Promise<string> {
    // Read key files to understand current state
    let projectState = '';
    
    try {
      // List files in working directory
      const files = fs.readdirSync(config.workingDir, { recursive: true }) as string[];
  ...
  private async generateChanges(
    ...
    try {
      if (fs.existsSync(promptMdPath)) {
        ...
        this.emit('log', sessionId, '📄 Using PROMPT.md from project directory');
      } else {
        // Try to get from database via promptMd service
        try {
          const dbPrompt = await promptMdService.getPrompt(parseInt(config.userId), config.workingDir);
          if (dbPrompt) {
...
`;
  ...
  private async applyChanges(
    ...
    if (codeResult.code) {
      this.emit('log', sessionId, `Generated code:\n${codeResult.code.substring(0, 500)}...`);
  ...
  private async runTests(
    ...
    return new Promise((resolve) => {
      ...
      ptyService.executeCommand(sessionId, 'pnpm test 2>&1 || npm test 2>&1 || echo "No test command found"');
  ...
  private async checkCompletion(
    ...
    for (const criterion of config.completionCriteria) {
      ...
      } else if (lowerCriterion.includes('build') && lowerCriterion.includes('succeed')) {
        // Check if build succeeds
        const buildOutput = this.outputBuffers.get(sessionId) || '';
        if (!buildOutput.includes('error') && !buildOutput.includes('failed')) criteriamet++;
      } else if (lowerCriterion.includes('typescript') && lowerCriterion.includes('error')) {
        // Check for TS errors
        const output = this.outputBuffers.get(sessionId) || '';
