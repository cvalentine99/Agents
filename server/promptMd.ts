 ...
 */
export async function initializePrompt(
  userId: number,
  projectPath: string
): Promise<ProjectPrompt> {
...
export function getSuggestedSigns(failurePattern: string): string[] {
  ...
  for (const [pattern, signs] of Object.entries(FAILURE_SIGNS)) {
    if (
      failurePattern.toLowerCase().includes(pattern.toLowerCase()) ||
...
export function detectFailurePattern(errorOutput: string): string {
  ...
  if (
    ...
    (lowerError.includes("fail") || lowerError.includes("error"))
  ) {
    return "test_failure";
  }
  if (
...
export async function listUserProjects(
  userId: number
): Promise<{ projectPath: string; version: number; updatedAt: Date }[]> {
  const db = await getDb();
  if (!db) return [];
