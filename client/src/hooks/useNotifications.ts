...
export function useNotifications() {
  ...
  const sendNotification = useCallback((
    ...
    const notification = new Notification(title, {
      ...
      requireInteraction: options?.requireInteraction ?? (type === 'loop_failed' || type === 'circuit_breaker_open'),
      badge: '/badge.png',
    });
  ...
  const notifyLoopComplete = useCallback((sessionName: string, iterations: number) => {
  ...
  const notifyLoopFailed = useCallback((sessionName: string, error: string) => {
    return sendNotification(
      'loop_failed',
      '❌ RALPH Loop Failed',
  ...
  const notifyCircuitBreakerOpen = useCallback((sessionName: string, reason: string) => {
    return sendNotification(
      'circuit_breaker_open',
  ...
  const notifyCircuitBreakerClosed = useCallback((sessionName: string) => {
    return sendNotification(
      'circuit_breaker_closed',
  ...
  const notifyIterationMilestone = useCallback((sessionName: string, iteration: number, total: number) => {
    return sendNotification(
      'iteration_milestone',
  ...
  const notifyStuckDetected = useCallback((sessionName: string, reason: string) => {
    return sendNotification(
      'stuck_detected',
      '🔴 Stuck Detected',
