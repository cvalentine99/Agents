    ...
    ws.onmessage = event => {
      try {
        ...
        switch (message.type) {
          ...
          case "iteration":
            addLog(
              `🔄 Iteration ${message.data.iteration} complete (${message.data.duration}ms)`
            );
            break;
      ...
      <div className="cyber-card p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full ${isConnected ? "bg-green-400" : "bg-red-400"} ${state.status === "RUNNING" ? "animate-pulse" : ""}`}
              />
              <span
                className={`font-mono text-sm font-bold ${getStatusColor(state.status)}`}
              >
                {state.status}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between p-4 bg-gray-800/50 rounded-lg mb-6">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`font-mono font-bold ${getBreakerColor(state.circuitBreaker)}`}
            >
        ...
        <div className="bg-gray-900 rounded-lg p-4 h-64 overflow-y-auto font-mono text-sm">
          {logs.length === 0 ? (
            <div className="text-gray-500">
              No logs yet. Start the RALPH Loop to see output.
            </div>
          ) : (
            logs.map((log, i) => (
          ...
          <TabsContent value="suggestions">
            <div className="space-y-4">
              <div className="text-sm text-slate-400 mb-4">
                <p className="mb-2">
                  When the RALPH Loop encounters repeated failures, the AI will
                  automatically suggest "signs" (guidance rules) to add to your
                  PROMPT.md.
                </p>
                <p>
                  Signs help tune the AI's behavior - like tuning a guitar. Each
                  failure is an opportunity to add guidance that prevents the
                  same mistake.
                </p>
              </div>
              {sessionId && parseInt(sessionId) > 0 ? (
