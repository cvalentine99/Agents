...
export function AgentProfiles({
  ...
  const customProfiles: ProfileData[] = (profilesQuery.data?.custom || []).map(
    p => {
  ...
  const handleEdit = (profile: ProfileData) => {
    ...
    const customProfile = profilesQuery.data?.custom.find(
      p => p.id === customId
    );
    if (customProfile) {
      ...
      setForm({
        ...
        outputStyle: customProfile.outputStyle as
          | "concise"
          | "detailed"
          | "balanced",
        codeGeneration: customProfile.codeGeneration as
          | "full"
          | "diffs"
          | "none",
        testingApproach: customProfile.testingApproach as
          | "test_first"
          | "test_after"
          | "no_tests",
      });
  ...
  const handleDuplicate = (profile: ProfileData) => {
    const sourceId = profile.isBuiltIn
      ? profile.id
      : parseInt(profile.id.replace("custom_", ""));
      ...
      <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
        <DialogContent className="max-w-2xl bg-slate-900 border-slate-700">
          <DialogHeader>
            ...
            <DialogDescription>
              Define a custom agent persona with specific behaviors and output
              styles.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                This prompt will be injected into every conversation with this
                agent.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v: "concise" | "detailed" | "balanced") =>
                    setForm({ ...form, outputStyle: v })
                  }
                >
                  <SelectTrigger className="bg-slate-800 border-slate-700">
                    <SelectValue />
              ...
              <div>
                ...
                <Select
                  value={form.codeGeneration}
                  onValueChange={(v: "full" | "diffs" | "none") =>
                    setForm({ ...form, codeGeneration: v })
                  }
                >
                  <SelectTrigger className="bg-slate-800 border-slate-700">
                    <SelectValue />
              ...
              <div>
                ...
                <Select
                  value={form.testingApproach}
                  onValueChange={(
                    v: "test_first" | "test_after" | "no_tests"
                  ) => setForm({ ...form, testingApproach: v })}
                >
                  <SelectTrigger className="bg-slate-800 border-slate-700">
                    <SelectValue />
                  </SelectTrigger>
      ...
      <Dialog open={showTemplateGallery} onOpenChange={setShowTemplateGallery}>
        <DialogContent className="max-w-5xl max-h-[85vh] overflow-y-auto bg-slate-900 border-slate-700">
          <DialogHeader>
            ...
            <DialogDescription>
              Browse pre-made agent profiles and import them to customize for
              your needs.
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <motion.div
      className={`relative p-4 rounded text-left transition-all ${
        isSelected
          ? "cyber-border"
          : "cyber-glass hover:border-[var(--cyber-purple)]/50"
      }`}
      style={{
    ...
    >
      ...
      {isSelected && (
        <motion.div
          ...
          animate={{ scale: 1 }}
        >
          <Check className="w-5 h-5" style={{ color: profile.color }} />
        </motion.div>
      )}
      ...
      >
        ...
        <div
          ...
          style={{
            color: profile.color,
            filter: isSelected
              ? `drop-shadow(0 0 10px ${profile.glowColor})`
              : undefined,
          }}
        >
          {profile.icon}
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {!profile.isBuiltIn && (
            <Badge
              variant="outline"
              className="text-[8px] px-1 py-0 border-cyan-500/50 text-cyan-400"
            >
              Custom
            </Badge>
          )}
