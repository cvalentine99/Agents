...
const BUILT_IN_TEMPLATES: PromptTemplate[] = [
  {
    ...
    context: "Using React 18+, TypeScript, and Tailwind CSS. Follow component best practices.",
    doneWhen: "Component renders correctly, has proper types, includes basic tests",
    doNot: "Don't use class components. Don't use inline styles. Don't skip TypeScript types.",
  ...
  },
  {
    ...
    description: "Build a new REST API endpoint with validation and error handling",
    goal: "Create a fully functional REST API endpoint",
    context: "Using Express/tRPC with Zod validation. Include proper error handling and logging.",
    doneWhen: "Endpoint works, has input validation, returns proper HTTP status codes, has tests",
    doNot: "Don't skip validation. Don't expose sensitive data. Don't forget error handling.",
  ...
  },
  {
    ...
    context: "Bug reported by user. Need to reproduce, identify root cause, and fix.",
    doneWhen: "Bug is fixed, regression test added, no new bugs introduced",
    doNot: "Don't refactor unrelated code. Don't change APIs. Don't skip testing the fix.",
  ...
  },
  {
    ...
    context: "Existing code works but needs improvement. Must not change external behavior.",
    doneWhen: "Code is cleaner, all tests pass, no behavior changes",
    doNot: "Don't change functionality. Don't break existing tests. Don't skip incremental commits.",
  ...
  },
  {
    ...
    context: "Using Vitest/Jest. Include unit, integration, and edge case tests.",
    doneWhen: "Coverage > 80%, all edge cases covered, tests are maintainable",
    doNot: "Don't write trivial tests. Don't mock everything. Don't skip error scenarios.",
  ...
  },
  {
    ...
    context: "New feature or system needs architectural planning before implementation.",
    doneWhen: "Architecture documented, tradeoffs explained, implementation plan ready",
    doNot: "Don't skip scalability considerations. Don't ignore security. Don't over-engineer.",
...
export default function PromptTemplates() {
  ...
  const allTemplates = useMemo(() => {
    const saved = (savedPrompts || []).map(p => ({
      ...
      doneWhen: p.doneWhen || "",
      doNot: p.doNot || "",
      tags: p.expandedPrompt?.split(",").map(t => t.trim()).filter(Boolean) || [],
      agentProfile: "",
  ...
  const handleLoadToDashboard = (template: PromptTemplate) => {
    ...
    localStorage.setItem("loadedPromptTemplate", JSON.stringify({
      goal: template.goal,
      context: template.context,
      doneWhen: template.doneWhen,
      doNot: template.doNot,
            ...
            <div className="flex items-center gap-2">
              <label className="cursor-pointer">
                <input
                  ...
                  className="hidden"
                />
                <Button variant="outline" size="sm" className="border-purple-500/30" asChild>
                  <span>
              ...
              <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                ...
                <DialogContent className="max-w-2xl bg-black/95 border-purple-500/30">
                  ...
                  <div className="grid gap-4 py-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Textarea
                        ...
                        onChange={e => setFormData(prev => ({ ...prev, doneWhen: e.target.value }))}
                    ...
                    <div className="grid gap-2">
                      ...
                      <Textarea
                        ...
                        onChange={e => setFormData(prev => ({ ...prev, doNot: e.target.value }))}
                  ...
                  <DialogFooter>
                    <Button variant="outline" onClick={() => { setIsCreateOpen(false); resetForm(); setEditingTemplate(null); }}>
                      Cancel
                    </Button>
                    <Button 
                    ...
                    >
                      {savePromptMutation.isPending ? "Saving..." : "Save Template"}
                    </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              ...
              <CardContent>
                ...
                {template.agentProfile && (
                  <div className="text-xs text-muted-foreground mb-4">
                    Agent: <span className="text-purple-400">{template.agentProfile.replace("_", " ")}</span>
                  </div>
                )}
